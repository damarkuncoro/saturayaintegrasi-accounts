    ...
    class LoginWithOauth
      ...
      def call(auth:, tenant:, ip_address: nil, user_agent: nil)
        ...
        if user.save
          ...
          Core::Result.success(user, meta: { session: session })
        else
          Core::Result.failure(user.errors.full_messages.to_sentence)
        end
      rescue => e
        Rails.logger.error "[Identity::LoginWithOauth] Error: #{e.message}"
        Core::Result.failure("Gagal masuk menggunakan #{auth.provider.to_s.titleize}.")
      end
    end
  end
end
