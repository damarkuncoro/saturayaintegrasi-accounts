  ...
  module Identity
    ...
    def execute(email:, password:, tenant:, ip_address: nil, user_agent: nil, trusted_device_fingerprint: nil)
      ...
      unless user
        ...
        return Core::Result.failure("Email atau kata sandi salah.")
      end
      ...
      if user.disabled_at.present? || !user.active?
        ...
        return Core::Result.failure("Akun Anda telah dinonaktifkan. Silakan hubungi dukungan pelanggan.")
      end
      ...
      if user.otp_required_for_login?
        ...
        unless is_trusted
          ...
          return Core::Result.success(user, meta: { status: :mfa_required })
        end
      end
      ...
      @audit_logger.log(action: "login_success", auditable: user, tenant: tenant, metadata: { session_id: session.id })
