        ...
        def execute(user:, otp_code:, tenant:, ip_address: nil, user_agent: nil, remember_device: false)
          ...
          if result.success?
            ...
            if remember_device
              ...
              # Pastikan Use Case TrustDevice dipanggil dengan benar (namespace-nya mungkin perlu disesuaikan jika nanti direfaktor juga)
              ::UseCases::Identity::TrustDevice.new.execute(
                user: user,
